...
function PointOfSale({products,createOrder,close}) {
  ...
  const [serviceType,setServiceType]=useState('En el lugar')
  ...
  const complete=()=>{const order=createOrder({customer,phone:'Mostrador',address:serviceType==='En el lugar'?'Consumo en restaurante':'Recoge en sucursal',serviceType,lines:lines.map(({id,name,price,qty,note,size,station})=>({id,name,price,qty,note,size,station})),subtotal,delivery:0,total:subtotal,payment,paid:true});notifyOrderByWhatsApp(order);close()}
  return <Modal title={reviewing?'Revisar e imprimir comanda':'Nueva venta en caja'} close={close} large>{!reviewing?<div className="pos-layout"><div><label className="field-label">Cliente<input value={customer} onChange={e=>setCustomer(e.target.value)}/></label><div className="service-options"><button className={serviceType==='En el lugar'?'active':''} onClick={()=>setServiceType('En el lugar')}><Store/> Consumo en el lugar</button><button className={serviceType==='Para llevar'?'active':''} onClick={()=>setServiceType('Para llevar')}><ShoppingBag/> Para llevar</button></div><div className="pos-products">{products.filter(p=>p.active).map(p=><button key={p.id} onClick={()=>add(p)}><ProductMedia product={p}/><b>{p.name}</b><small>{money(p.price)}</small>{p.deliveryEnabled===false&&<em>Solo local</em>}</button>)}</div></div><aside><h3>Detalle de venta</h3><small className="pos-service-label">{serviceType} · No se enviará a Reparto</small>{lines.length===0?<Empty text="Selecciona productos"/>:lines.map(line=><div className="pos-line-wrap" key={line.id}><div className="pos-line"><div className="pos-qty"><button type="button" onClick={()=>changeLineQty(line.id,-1)} aria-label={`Reducir ${line.name}`}><Minus/></button><span>{line.qty}</span><button type="button" onClick={()=>changeLineQty(line.id,1)} aria-label={`Aumentar ${line.name}`}><Plus/></button></div><b>{line.name}</b><strong>{money(line.price*line.qty)}</strong></div>{line.category==='Pizzas'&&<label className="pos-size">Tamaño<select value={line.size} onChange={e=>updateLineSize(line.id,e.target.value)}>{Object.keys(pizzaSizeAdjustments).map(size=><option key={size}>{size}</option>)}</select></label>}<input value={line.note||''} onChange={e=>updateLineNote(line.id,e.target.value)} placeholder="Indicaciones: sin salsa, sin cebolla..."/></div>)}<div className="pos-total"><span>Total</span><b>{money(subtotal)}</b></div><div className="payment-options compact"><button className={payment==='Efectivo'?'active':''} onClick={()=>setPayment('Efectivo')}><Wallet/> Efectivo</button><button className={payment==='Tarjeta'?'active':''} onClick={()=>setPayment('Tarjeta')}><CreditCard/> Tarjeta</button></div><button className="primary wide" disabled={!lines.length} onClick={()=>{setReviewing(true);setPrinted(false)}}>Revisar venta y comanda <ArrowRight/></button></aside></div>:<div className="sale-review"><section className="kitchen-print"><div className="ticket-logo">🔥 FUEGO</div><h2>COMANDA DE COCINA</h2><p className="ticket-meta">{new Date().toLocaleString('es-MX')} · Caja · {serviceType}</p><div className="ticket-customer"><small>CLIENTE</small><b>{customer}</b></div>{kitchenLines.length?<div className="ticket-lines">{kitchenLines.map(line=><article key={line.id}><b>{line.qty}× {line.name}{line.size?` · ${line.size}`:''}</b>{line.note&&<strong>INDICACIÓN: {line.note}</strong>}</article>)}</div>:<div className="no-kitchen-ticket"><PackageCheck/><b>Sin productos para cocina</b><small>Todo el pedido se enviará directamente a Barra / Despacho.</small></div>}<footer><span>Productos cocina: {kitchenLines.reduce((sum,line)=>sum+line.qty,0)}</span><b>Total venta: {money(subtotal)}</b></footer></section><aside className="review-actions"><span className="eyebrow">Paso final</span><h2>Confirma la venta</h2><p className="review-service"><Store/> {serviceType} · Sin reparto</p><ol><li className="done"><Check/> Productos revisados</li><li className={printed||!kitchenLines.length?'done':''}><Printer/> {kitchenLines.length?'Imprimir ticket de cocina':'No requiere impresión de cocina'}</li><li><CircleDollarSign/> Confirmar cobro y enviar pedido</li></ol>{kitchenLines.length&&<button className="outline wide" onClick={printKitchenTicket}><Printer/> {printed?'Imprimir nuevamente':'Imprimir ticket de cocina'}</button>}<button className="primary wide" disabled={kitchenLines.length&&!printed} onClick={complete}>Confirmar venta y enviar <ArrowRight/></button><button className="review-back" onClick={()=>setReviewing(false)}>Volver a editar la venta</button></aside></div>}</Modal>
}
