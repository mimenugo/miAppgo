...
function DispatchModule({store}) {
  const orders=store.orders.filter(o=>isDeliveryOrder(o)&&['Nuevo','En barra','En cocina','Listo','Asignado','En ruta'].includes(o.status))
  const assignDriver=(order,driver)=>store.updateOrder(order.id,{driver,status:driver&&order.status==='Listo'?'Asignado':order.status==='Asignado'&&!driver?'Listo':order.status})
...
function PointOfSale({products,createOrder,close}) {
  ...
  const barLines=lines.filter(line=>(line.station||'Cocina')==='Barra')
  const serviceReady=serviceType==='Para llevar'||(deliveryData.phone.trim()&&deliveryData.address.trim())
  const printOrderTicket=()=>{setPrinted(true);setTimeout(()=>window.print(),80)}
  const complete=()=>{const order=createOrder({customer,phone:serviceType==='Domicilio'?deliveryData.phone:'Mostrador',address:serviceType==='Domicilio'?deliveryData.address:'',serviceType,lines:lines.map(({id,name,price,qty,note,size,station})=>({id,name,price,qty,note,size,station})),subtotal,delivery:0,total:subtotal,payment,paid:true});notifyOrderByWhatsApp(order);close()}
  return <Modal title={reviewing?'Revisar e imprimir pedido':'Nueva venta en caja'} close={close} large>{!reviewing?<div className="pos-layout"><div><label className="field-label">Cliente<input value={customer} onChange={e=>setCustomer(e.target.value)}/></label><div className="service-options"><button className={serviceType==='Para llevar'?'active':''} onClick={()=>changeServiceType('Para llevar')}><ShoppingBag/> Para llevar</button><button className={serviceType==='Domicilio'?'active':''} onClick={()=>changeServiceType('Domicilio')}><Bike/> Entrega a domicilio</button></div>{serviceType==='Domicilio'&&<div className="pos-delivery-fields"><label>Teléfono<input value={deliveryData.phone} onChange={e=>setDeliveryData({...deliveryData,phone:e.target.value})} placeholder="Teléfono del cliente"/></label><label>Dirección de entrega<input value={deliveryData.address} onChange={e=>setDeliveryData({...deliveryData,address:e.target.value})} placeholder="Calle, número y colonia"/></label></div>}<div className="pos-products">{products.filter(p=>p.active&&(serviceType!=='Domicilio'||p.deliveryEnabled!==false)).map(p=><button key={p.id} onClick={()=>add(p)}><ProductMedia product={p}/><b>{p.name}</b><small>{money(p.price)}</small>{p.deliveryEnabled===false&&<em>Solo para llevar</em>}</button>)}</div></div><aside><h3>Detalle de venta</h3><small className="pos-service-label">{serviceType==='Domicilio'?'Entrega a domicilio · Se enviará a Reparto':'Para llevar · No se enviará a Reparto'}</small>{lines.length===0?<Empty text="Selecciona productos"/>:lines.map(line=><div className="pos-line-wrap" key={line.id}><div className="pos-line"><div className="pos-qty"><button type="button" onClick={()=>changeLineQty(line.id,-1)} aria-label={`Reducir ${line.name}`}><Minus/></button><span>{line.qty}</span><button type="button" onClick={()=>changeLineQty(line.id,1)} aria-label={`Aumentar ${line.name}`}><Plus/></button></div><b>{line.name}</b><strong>{money(line.price*line.qty)}</strong></div>{line.category==='Pizzas'&&<label className="pos-size">Tamaño<select value={line.size} onChange={e=>updateLineSize(line.id,e.target.value)}>{Object.keys(pizzaSizeAdjustments).map(size=><option key={size}>{size}</option>)}</select></label>}<input value={line.note||''} onChange={e=>updateLineNote(line.id,e.target.value)} placeholder="Indicaciones: sin salsa, sin cebolla..."/></div>)}<div className="pos-total"><span>Total</span><b>{money(subtotal)}</b></div><div className="payment-options compact"><button className={payment==='Efectivo'?'active':''} onClick={()=>setPayment('Efectivo')}><Wallet/> Efectivo</button><button className={payment==='Tarjeta'?'active':''} onClick={()=>setPayment('Tarjeta')}><CreditCard/> Tarjeta</button></div><button className="primary wide" disabled={!lines.length||!serviceReady} onClick={()=>{setReviewing(true);setPrinted(false)}}>Revisar venta y ticket <ArrowRight/></button></aside></div>:<div className="sale-review"><section className="kitchen-print"><div className="ticket-logo">🔥 FUEGO</div><h2>TICKET DEL PEDIDO</h2><p className="ticket-meta">{new Date().toLocaleString('es-MX')} · Caja · {serviceType}</p><div className="ticket-customer"><small>CLIENTE</small><b>{customer}</b></div>{serviceType==='Domicilio'&&<div className="ticket-delivery"><b>{deliveryData.phone}</b><span>{deliveryData.address}</span></div>}<div className="ticket-groups">{kitchenLines.length>0&&<section><h3>COCINA</h3><div className="ticket-lines">{kitchenLines.map(line=><article key={line.id}><b>{line.qty}× {line.name}{line.size?` · ${line.size}`:''}</b>{line.note&&<strong>INDICACIÓN: {line.note}</strong>}</article>)}</div></section>}{barLines.length>0&&<section><h3>BARRA / DESPACHO</h3><div className="ticket-lines">{barLines.map(line=><article key={line.id}><b>{line.qty}× {line.name}{line.size?` · ${line.size}`:''}</b>{line.note&&<strong>INDICACIÓN: {line.note}</strong>}</article>)}</div></section>}</div><footer><span>Productos totales: {lines.reduce((sum,line)=>sum+line.qty,0)}</span><b>Total venta: {money(subtotal)}</b></footer></section><aside className="review-actions"><span className="eyebrow">Paso final</span><h2>Confirma la venta</h2><p className="review-service">{serviceType==='Domicilio'?<Bike/>:<ShoppingBag/>} {serviceType}{serviceType==='Domicilio'?' · Con reparto':' · Sin reparto'}</p><ol><li className="done"><Check/> Productos revisados</li><li className={printed||!kitchenLines.length?'done':''}><Printer/> {printed?'Ticket completo impreso':kitchenLines.length?'Imprimir ticket completo':'Impresión opcional del ticket'}</li><li><CircleDollarSign/> Confirmar cobro y enviar pedido</li></ol><button className="outline wide" onClick={printOrderTicket}><Printer/> {printed?'Imprimir nuevamente':'Imprimir ticket completo'}</button><button className="primary wide" disabled={!serviceReady||(kitchenLines.length&&!printed)} onClick={complete}>Confirmar venta y enviar <ArrowRight/></button><button className="review-back" onClick={()=>setReviewing(false)}>Volver a editar la venta</button></aside></div>}</Modal>
}
...
function DriverView({store}) {
  ...
  const activeOrders=deliveryOrders.filter(o=>['Nuevo','En barra','En cocina','Listo','Asignado','En ruta'].includes(o.status))
