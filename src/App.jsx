...
const requestCashDrawer = (order, settings) => {
  ...
  window.dispatchEvent(
    new CustomEvent("gastropos:open-cash-drawer", {
      detail: { orderId: order.id, printer: settings.cashPrinter },
    }),
  );
  return true;
};
...
const roles = {
  administrador: {
    ...
    subtitle: "Acceso completo al restaurante",
  },
  cajero: {
    label: "Cajero POS",
    icon: CircleDollarSign,
    subtitle: "Ventas, caja y cortes",
  },
  barra: {
    label: "Barra / Despacho",
    icon: PackageCheck,
    subtitle: "Productos listos y entrega",
  },
  repartidor: {
      ...
      <section className="session-window">
        ...
        {selected && (
          <div className="login-form">
            ...
            <label>
              Contraseña
              <input type="password" value={password} onChange={(event) => setPassword(event.target.value)} autoComplete="current-password" onKeyDown={(event) => event.key === "Enter" && signIn()} />
            </label>
            {message && <p>{message}</p>}
...
function CustomerView({ products, cart, addItem, changeQty, updateNote, updateSize, showCart, setShowCart, createOrder, createStripeCheckout, business, posSettings }) {
  ...
  return (
    <>
        ...
        <section className="hero">
          <div className="hero-copy">
            <span className="eyebrow">
              <Sparkles size={15} /> Sabor local, directo a tu puerta
            </span>
            <h1>
              Tu antojo merece llegar <em>caliente.</em>
            </h1>
        ...
        <section className="menu-section" id="menu">
          <div className="section-heading">
            ...
            <label className="search">
              <Search size={18} />
              <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Buscar platillo" />
            </label>
          </div>
              ...
              <article className="product-card" key={product.id}>
                ...
                <div className="product-info">
                  ...
                  <footer>
                    ...
                    <button onClick={() => addItem(product)} aria-label={`Agregar ${product.name}`}>
                      <Plus size={19} />
                    </button>
                  </footer>
                </div>
      ...
      {showCart && <Checkout cart={cart} changeQty={changeQty} updateNote={updateNote} updateSize={updateSize} close={() => setShowCart(false)} createOrder={createOrder} createStripeCheckout={createStripeCheckout} business={business} posSettings={posSettings} />}
    </>
  );
}

function Checkout({ cart, changeQty, updateNote, updateSize, close, createOrder, createStripeCheckout, business, posSettings }) {
  ...
  const subtotal = cart.reduce((sum, row) => sum + row.price * row.qty, 0),
    delivery = serviceType === "Domicilio" ? 39 : 0,
    total = subtotal + delivery;
  const valid = form.customer.trim() && (serviceType === "Recoger en restaurante" || (form.phone.trim() && form.address.trim()));
  const confirm = async () => {
    try {
      const order = await createOrder({
        ...
        source: "pwa",
        serviceType,
        lines: cart.map(({ id, name, price, qty, note, size, station }) => ({
          id,
          name,
          price,
          qty,
      ...
      <aside className="cart-panel">
        ...
        {step === "cart" && (
          <>
              ...
              {cart.length === 0 ? (
                ...
                cart.map((row) => (
                  <div className="cart-row" key={row.id}>
                    <span className={`mini ${row.tone}`}>{row.emoji}</span>
                    <div>
                      <b>{row.name}</b>
                      <small>{money(row.price)}</small>
                    </div>
                    <div className="qty">
                      <button onClick={() => changeQty(row.id, -1)} aria-label={`Reducir ${row.name}`}>
                        <Minus size={14} />
                      </button>
                      <span>{row.qty}</span>
                      <button onClick={() => changeQty(row.id, 1)} aria-label={`Aumentar ${row.name}`}>
                        <Plus size={14} />
                      </button>
                    </div>
                    {row.category === "Pizzas" && (
                    ...
                    <label className="item-note">
                      ...
                      <input value={row.note || ""} onChange={(e) => updateNote(row.id, e.target.value)} placeholder="Ej. sin salsa, sin tomate, sin cebolla" />
                    </label>
                  </div>
                ))
              )}
            ...
            {cart.length > 0 && (
              <OrderTotal subtotal={subtotal} delivery={delivery} total={total}>
                <button className="primary wide" onClick={() => setStep("checkout")}>
        ...
        {step === "checkout" && (
          <div className="checkout-form">
            <div className="step-title">
              <span>1</span>
              <h3>Tipo y datos de entrega</h3>
            </div>
            <div className="service-options">
              <button className={serviceType === "Domicilio" ? "active" : ""} onClick={() => setServiceType("Domicilio")}>
                <Bike /> Entrega a domicilio
              </button>
              <button className={serviceType === "Recoger en restaurante" ? "active" : ""} onClick={() => setServiceType("Recoger en restaurante")}>
            ...
            <OrderTotal subtotal={subtotal} delivery={delivery} total={total}>
              <button className="primary wide" disabled={!valid} onClick={confirm}>
...
function OrderTotal({ subtotal, delivery, total, children }) {
  return (
    ...
    <main className="app-shell">
      ...
      <div className="workspace">
        <div className="workspace-head">
          ...
          {onNewSale && (
            <button className="outline" onClick={onNewSale}>
              <Plus /> Nueva venta
            </button>
          )}
        </div>
...
function CashierView({ store }) {
  ...
  return (
    <DashboardShell active={active} setActive={setActive} moduleItems={cashierModules} onNewSale={() => setSaleOpen(true)} title={active === "Caja" ? "Control y arqueo de caja" : "Pedidos de caja"} subtitle="Punto de venta y control del turno actual.">
      {active === "Caja" && <CashControl store={store} />}
      {active === "Pedidos" && <OrdersModule store={store} />}
      {saleOpen && <PointOfSale store={store} close={() => setSaleOpen(false)} />}
    </DashboardShell>
...
function Summary({ store }) {
  const sales = store.orders.reduce((sum, o) => sum + o.total, 0),
    active = store.orders.filter((o) => !["Entregado", "Cancelado"].includes(o.status)).length;
  const stats = [
    ["Ventas registradas", money(sales), CircleDollarSign, "orange"],
    ["Pedidos", store.orders.length, ShoppingBag, "blue"],
    ["En proceso", active, CookingPot, "green"],
    ["Clientes", store.customers.length, Users, "purple"],
  ];
  return (
    <>
      <div className="stats">
...
function OrderRow({ order }) {
  return (
    <div className="order-row">
      ...
      <span className={`status ${order.status.toLowerCase().replaceAll(" ", "-")}`}>{order.status}</span>
    ...
    <section className="dash-card data-card">
      ...
      <div className="data-table">
        {store.orders.map((order) => (
          <div className={`data-row order-data ${Date.now() - Number(order.createdTimestamp || 0) < 60000 ? "new-order-alert" : ""}`} key={`${order.dbId}-${order.id}`}>
            <b>#{order.id}</b>
            <div>
            ...
            <span>
              {order.payment}
              <small>{order.paymentStatus || (order.paid ? "Pagado" : "Pendiente de pago")}</small>
            </span>
            <b>{money(order.total)}</b>
            <select value={order.status} onChange={(e) => store.updateOrder(order.id, { status: e.target.value })}>
              {statuses.map((status) => (
...
function ProductionBoard({ store }) {
  ...
  return (
    <div className="kanban">
      {columns.map(([areaState, label, className]) => (
        <section className={className} key={areaState}>
          <header>
            <h2>{label}</h2>
            <span>{visible.filter((order) => areaStatus(order, "Cocina") === areaState).length}</span>
          </header>
          {visible
            .filter((order) => areaStatus(order, "Cocina") === areaState)
            .map((order) => (
              <article className="ticket" key={order.id}>
                <div>
                  <b>#{order.id}</b>
                  <span>
                    <Clock3 /> {order.createdAt}
                  </span>
                </div>
                ...
                {areaState !== "Listo" && (
                  <footer>
                    ...
                    <button onClick={() => updatePreparationArea(store, order, "Cocina", areaState === "Pendiente" ? "Preparando" : "Listo")}>
                      {areaState === "Pendiente" ? "Aceptar" : "Terminar cocina"} <Check />
                    </button>
                  </footer>
                )}
              </article>
...
function BarBoard({ store }) {
  ...
  const Card = ({ order, done = false }) => {
    const state = areaStatus(order, "Barra");
    const ready = ["Listo para recoger", "Listo para enviar", "Listo"].includes(order.status);
    return (
      <article className={`bar-ticket ${done ? "prepared" : ""}`}>
        <div className="bar-ticket-head">
          <div>
            <small>#{order.id}</small>
            <h3>{order.customer}</h3>
          </div>
          <span className={`status ${done ? "entregado" : "en-barra"}`}>{done ? "Listo" : state === "Preparando" ? "Preparando" : "Nuevo"}</span>
        </div>
        ...
        <footer>
          <span>{isDeliveryOrder(order) ? "Pedido a domicilio" : order.serviceType || "Recoger en restaurante"}</span>
          {!done ? (
            <button className="primary" onClick={() => updatePreparationArea(store, order, "Barra", state === "Pendiente" ? "Preparando" : "Listo")}>
              {state === "Pendiente" ? "Aceptar" : "Marcar listo"} <Check />
            </button>
          ) : !ready ? (
            <b className="bar-waiting">
              <CookingPot /> Esperando cocina
            </b>
          ) : !isDeliveryOrder(order) ? (
            <button className="outline" onClick={() => store.updateOrder(order.id, { status: "Entregado" })}>
              Entregar al cliente
            </button>
          ) : (
            <b className="bar-ready">
              <Bike /> Listo para reparto
            </b>
          )}
        </footer>
      </article>
  ...
  return (
    <div className="bar-board">
      <section>
        <header>
          <div>
            <small>BARRA / LISTO PARA SERVIR</small>
            <h2>Por preparar / despachar</h2>
          </div>
          <span>{pending.length}</span>
        </header>
        <div className="bar-grid">{pending.length ? pending.map((order) => <Card order={order} key={order.id} />) : <Empty text="No hay productos pendientes en barra" />}</div>
      </section>
      <section>
        <header>
          <div>
          ...
          <span>{prepared.length}</span>
        </header>
        <div className="bar-grid">{prepared.length ? prepared.map((order) => <Card order={order} done key={order.id} />) : <Empty text="No hay pedidos preparados" />}</div>
      </section>
    </div>
  );
}
...
function DispatchModule({ store }) {
  ...
  const assignDriver = (order, driver) =>
    store.updateOrder(order.id, {
      driver,
      status: driver && readyStatus(order.status) ? "Asignado a repartidor" : order.status === "Asignado a repartidor" && !driver ? "Listo para enviar" : order.status,
    });
  return (
    <section className="dash-card data-card">
      <div className="card-title">
      ...
      {orders.length === 0 ? (
        <Empty text="No hay pedidos para reparto" />
      ) : (
            ...
            <article key={order.id}>
              <div>
                ...
                <div>
                  ...
                  {!order.driver && <small>Pendiente de asignar</small>}
                </div>
              </div>
              <div>
...
function CashControl({ store }) {
  ...
  const [shiftName, setShiftName] = useState(store.posSettings.shifts?.[0]?.name || "General");
  ...
  const addMovement = () => {
    if (Number(movement.amount) > 0 && movement.concept.trim()) {
      store.addCashMovement(movement);
      setMovement({ ...movement, amount: "", concept: "" });
    }
  };
  const closeRegister = (closeType) => {
    if (counted === "" || cash.pendingPayments?.length) return;
    ...
    setCounted("");
    setNote("");
    setTab("Historial de cortes");
  };
  const saveSettings = () => {
    store.savePosSettings(posSettings);
  ...
  return (
    <div className="cash-module">
            ...
            {cash.open ? (
              ...
              <div className="cash-open-form">
                <select value={shiftName} onChange={(e) => setShiftName(e.target.value)}>
                  {(store.posSettings.shifts || [{ name: "General" }]).map((item) => (
                    <option key={item.name}>{item.name}</option>
                  ))}
                </select>
                <input type="number" min="0" value={opening} onChange={(e) => setOpening(e.target.value)} />
                <button className="primary" onClick={() => store.openCashRegister(opening, shiftName)}>
                  <LogIn /> Abrir caja
                </button>
              </div>
            )}
          ...
          {cash.open && (
            <div className="cash-summary">
              <article>
                ...
                <h3>{money(cash.openingAmount)}</h3>
              </article>
              <article>
                <small>Ventas en efectivo</small>
                <h3>{money(cashSales)}</h3>
              </article>
              <article>
                <small>Entradas y retiros</small>
                <h3>{money(inputs - withdrawals)}</h3>
              </article>
              ...
              <article>
                ...
                <h3>$1 USD = {money(store.posSettings.usdExchangeRate || 0)}</h3>
              </article>
            </div>
          )}
          <section className="dash-card cash-breakdown">
            <div className="card-title">
              <div>
                <small>Resumen del turno</small>
            ...
            <p>
              <span>Entradas</span>
              <b className="positive">+ {money(inputs)}</b>
            </p>
            <p>
              <span>Retiros</span>
      ...
      {tab === "Entradas / retiros" && (
        <div className="cash-two-columns">
          <section className="dash-card">
            <div className="card-title">
              <div>
                <small>Nuevo movimiento</small>
                <h2>Registrar entrada o retiro</h2>
              </div>
            </div>
            <div className="cash-movement-form">
              ...
              <label>
                Monto
                <input type="number" min="0" value={movement.amount} onChange={(e) => setMovement({ ...movement, amount: e.target.value })} placeholder="0.00" />
              </label>
              <label className="full">
                Concepto
                <input value={movement.concept} onChange={(e) => setMovement({ ...movement, concept: e.target.value })} placeholder="Ej. pago a proveedor, fondo adicional" />
              </label>
              <button className="primary full" disabled={!cash.open || !movement.concept || Number(movement.amount) <= 0} onClick={addMovement}>
                Guardar movimiento
              </button>
            </div>
          </section>
      ...
      {["Cierre de turno (X)", "Cierre del día (Z)"].includes(tab) && (
        <section className="dash-card close-register">
          <div className="card-title">
            <div>
              <small>Arqueo {tab.endsWith("(X)") ? "por turno" : "final diario"}</small>
              <h2>{tab}</h2>
            </div>
            <span className={`status ${cash.open ? "en-ruta" : "entregado"}`}>{cash.open ? "Pendiente" : "Cerrada"}</span>
          </div>
          {cash.open ? (
            <>
              <div className="close-grid">
                <div>
                  ...
                  <p>Incluye fondo, ventas y movimientos.</p>
                </div>
                <label>
                  Efectivo contado
                ...
                <label className="full">
                  Observaciones
                  <input value={note} onChange={(e) => setNote(e.target.value)} placeholder="Notas del cierre o aclaraciones" />
                </label>
              </div>
              {cash.pendingPayments?.length > 0 && (
                <div className="api-warning">
                  No se puede cerrar: {cash.pendingPayments.length} pedido(s) sin cobrar: {cash.pendingPayments.map((item) => item.id).join(", ")}
                </div>
              )}
              {counted !== "" && (
                <div className={`difference ${Number(counted) - expected === 0 ? "balanced" : Number(counted) - expected > 0 ? "positive" : "negative"}`}>
                  <span>Diferencia de caja</span>
                  <b>{money(Number(counted) - expected)}</b>
                </div>
              )}
              <button className="primary" disabled={counted === "" || cash.pendingPayments?.length > 0} onClick={() => closeRegister(tab.endsWith("(X)") ? "X" : "Z")}>
                Confirmar cierre {tab.endsWith("(X)") ? "X" : "Z"}
              </button>
            </>
          ) : (
            <Empty text="La caja ya fue cerrada. Revisa el historial o abre un nuevo turno." />
          )}
        </section>
      )}
      {tab === "Historial de cortes" && (
        <section className="dash-card">
          <div className="card-title">
            <div>
              <small>Cortes Z</small>
              <h2>Historial de cierres</h2>
            </div>
            <b>{cash.cuts.length} cortes</b>
          </div>
              ...
              cash.cuts.map((cut) => (
                <article key={cut.id}>
                  ...
                  <div>
                    ...
                    <b>{money(cut.expected)}</b>
                  </div>
                  <div>
                    <small>Contado</small>
                    <b>{money(cut.counted)}</b>
                  </div>
                  <div>
                    <small>Diferencia</small>
                    <b className={cut.difference === 0 ? "" : cut.difference > 0 ? "positive" : "negative"}>{money(cut.difference)}</b>
                  </div>
                </article>
              ))
            )}
          ...
          <div className="settings-grid">
            <fieldset>
              ...
              <label>
                Impresora de caja
                <input
                  value={posSettings.cashPrinter}
                  onChange={(e) =>
                    setPosSettings({
                      ...posSettings,
                      cashPrinter: e.target.value,
                    })
                  }
                />
              </label>
              <label>
                Impresora de cocina
                <input
                  value={posSettings.kitchenPrinter}
                  onChange={(e) =>
                    setPosSettings({
                      ...posSettings,
              ...
              <label className="inline-check">
                <input
                  type="checkbox"
                  checked={posSettings.kitchenPrinterEnabled}
                  onChange={(e) =>
                    setPosSettings({
                      ...posSettings,
                      kitchenPrinterEnabled: e.target.checked,
                    })
                  }
                />{" "}
              ...
              <label className="inline-check">
                <input
                  type="checkbox"
                  checked={posSettings.barPrinterEnabled}
                  onChange={(e) =>
                    setPosSettings({
                      ...posSettings,
                      barPrinterEnabled: e.target.checked,
                    })
                  }
                />{" "}
            ...
            <fieldset>
              ...
              <label className="inline-check">
                <input
                  type="checkbox"
                  checked={posSettings.cashDrawerEnabled}
                  onChange={(e) =>
                    setPosSettings({
                      ...posSettings,
                ...
                />{" "}
                Abrir al cobrar en efectivo
              </label>
              <label className="inline-check">
                <input
                  type="checkbox"
                  checked={posSettings.cashDrawerCompatible}
                  onChange={(e) =>
                    setPosSettings({
                      ...posSettings,
                      cashDrawerCompatible: e.target.checked,
                    })
                  }
                />{" "}
            ...
            <fieldset>
              ...
              <label>
                Titular
                <input
                  value={posSettings.accountHolder}
                  onChange={(e) =>
                    setPosSettings({
                      ...posSettings,
            ...
            <fieldset>
              <legend>Pedidos programados</legend>
              <div className="settings-row">
                <label>
                  Apertura
                  <input
                    type="time"
                    value={posSettings.openingTime}
                    onChange={(e) =>
                      setPosSettings({
                        ...posSettings,
              ...
              <div className="settings-row">
                ...
                <label>
                  ...
                  <input
                    type="number"
                    min="1"
                    value={posSettings.slotCapacity}
                    onChange={(e) =>
                      setPosSettings({
                        ...posSettings,
...
function ProductsModule({ store }) {
  ...
  const save = async () => {
    if (editing.name && Number(editing.price) > 0) {
      try {
        await store.saveProduct({ ...editing, price: Number(editing.price) });
        setEditing(null);
      } catch (error) {
        alert(error.message);
      }
  ...
  return (
    <>
      <div className="module-toolbar">
        <p>{store.products.length} productos registrados</p>
          ...
          <article key={product.id}>
            ...
            <div>
              ...
              <div className="product-badges">
                <em className={`station-badge ${product.station === "Barra" ? "bar" : product.station === "Ambas" ? "both" : ""}`}>{stationLabel(product.station)}</em>
                {product.deliveryEnabled === false && <em className="station-badge local-only">Solo venta local</em>}
              </div>
            </div>
            <label className="switch">
              <input type="checkbox" checked={product.active} onChange={() => store.saveProduct({ ...product, active: !product.active })} />
              <i />
            </label>
            <button className="icon-edit" onClick={() => setEditing(product)}>
              <Edit3 />
            </button>
          </article>
        ))}
      ...
      {editing && (
        <Modal title={editing.id ? "Editar producto" : "Nuevo producto"} close={() => setEditing(null)}>
          <div className="form-grid">
            <label>
              Nombre
            ...
            <label>
              ...
              <select value={editing.station || "Cocina"} onChange={(e) => setEditing({ ...editing, station: e.target.value })}>
                ...
                <option value="Ambas">Cocina y Barra</option>
              </select>
            </label>
            ...
            <label className="full image-field">
              ...
              {editing.image && (
                <div className="image-preview">
                  ...
                  <button type="button" onClick={() => setEditing({ ...editing, image: "" })}>
          ...
          <button className="primary wide modal-save" onClick={save}>
            Guardar producto
          </button>
        </Modal>
      )}
...
function BusinessSettings({ store }) {
  ...
  const loadLogo = (file) => {
    if (!file) return;
    if (file.size > 1500000) {
      setMessage("El logotipo debe pesar menos de 1.5 MB.");
      return;
    }
    const reader = new FileReader();
    reader.onload = () =>
      ...
      <section className="dash-card">
        ...
        <div className="business-form">
          ...
          <label className="full business-logo-field">
            ...
            {form.logoUrl && <img src={form.logoUrl} alt="Logotipo actual" />}
          </label>
        </div>
...
function ConfigurationCenter({ store }) {
  ...
  useEffect(() => {
    if (!settingsDirty.current) setSettings(store.posSettings);
  }, [store.posSettings]);
  const set = (key, value) => {
  ...
  const selectedTime = new Intl.DateTimeFormat("es-MX", {
    timeZone: store.business.timezone,
    dateStyle: "short",
    timeStyle: "medium",
  }).format(new Date());
  const systemZone = Intl.DateTimeFormat().resolvedOptions().timeZone;
  const systemTime = new Intl.DateTimeFormat("es-MX", {
    dateStyle: "short",
    timeStyle: "medium",
  }).format(new Date());
  const zoneMatches = ["America/Tijuana", "America/Los_Angeles"].includes(store.business.timezone) && ["America/Tijuana", "America/Los_Angeles", "Etc/GMT+8"].includes(systemZone);
  return (
    <div className="cash-module">
      <div className="cash-tabs config-tabs">
        {tabs.map((item) => (
          <button key={item} className={tab === item ? "active" : ""} onClick={() => setTab(item)}>
            {item}
          </button>
        ...
        <section className="dash-card pos-settings">
          ...
          <div className="settings-grid">
            <fieldset>
              ...
              <label>
                Cocina
                <input value={settings.kitchenPrinter || ""} onChange={(e) => set("kitchenPrinter", e.target.value)} />
              </label>
              <label>
                Barra
                <input value={settings.barPrinter || ""} onChange={(e) => set("barPrinter", e.target.value)} />
              </label>
            </fieldset>
            <fieldset>
              <legend>Cajón</legend>
            ...
            <fieldset>
              ...
              <label className="inline-check">
                <input type="checkbox" checked={settings.showUsdOnTicket !== false} onChange={(e) => set("showUsdOnTicket", e.target.checked)} /> Mostrar total USD aproximado en ticket
              </label>
            </fieldset>
          </div>
      ...
      {tab === "Turnos" && (
        <section className="dash-card pos-settings">
          <div className="card-title">
            <div>
              <small>Cierres X</small>
              <h2>Horarios aproximados por turno</h2>
            </div>
          </div>
          <div className="settings-grid">
            {(settings.shifts || []).map((shift, index) => (
              <fieldset key={index}>
                <legend>Turno {index + 1}</legend>
                <label>
                  Nombre
                  <input
                    value={shift.name}
                    onChange={(e) =>
                      set(
                        "shifts",
                        settings.shifts.map((item, i) => (i === index ? { ...item, name: e.target.value } : item)),
                      )
                    }
                  />
                </label>
                  ...
                  <label>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        set(
                          "shifts",
                          settings.shifts.map((item, i) => (i === index ? { ...item, start: e.target.value } : item)),
                        )
                      }
                    />
                  </label>
                  <label>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        set(
                          "shifts",
                          settings.shifts.map((item, i) => (i === index ? { ...item, end: e.target.value } : item)),
                        )
                      }
                    />
                  </label>
      ...
      {tab === "Zona horaria" && (
        <section className="dash-card">
          <div className="card-title">
            <div>
              <small>Reloj oficial del negocio</small>
              <h2>Zona horaria</h2>
            </div>
          </div>
          <div className="business-form">
          ...
          <div className="settings-grid">
            <fieldset>
              ...
              <label className="inline-check">
                <input
                  type="checkbox"
                  checked={Boolean(settings.stripe?.enabled)}
                  onChange={(e) =>
                    set("stripe", {
                      ...settings.stripe,
              ...
              <label>
                Ambiente
                <select
                  value={settings.stripe?.environment || "test"}
                  onChange={(e) =>
                    set("stripe", {
                      ...settings.stripe,
              ...
              <label>
                ...
                <input
                  ...
                  onChange={(e) =>
                    set("stripe", {
                      ...settings.stripe,
                      publishableTest: e.target.value,
                    })
                  }
              ...
              <label>
                ...
                <input
                  ...
                  onChange={(e) =>
                    set("stripe", {
                      ...settings.stripe,
                      publishableLive: e.target.value,
                    })
                  }
      ...
      {tab === "QR y transferencias" && (
        <section className="dash-card pos-settings">
          <div className="card-title">
            <div>
              <small>Pagos por validar</small>
              <h2>Transferencia, QR, CoDi y DiMo</h2>
            </div>
          </div>
          <div className="settings-grid">
            <fieldset>
              ...
              {[
                ...
                <label className="inline-check" key={key}>
                  <input
                    type="checkbox"
                    checked={Boolean(settings.payments?.[key])}
                    onChange={(e) =>
                      set("payments", {
                        ...settings.payments,
            ...
            <fieldset>
              ...
              <label>
                Titular
                <input value={settings.accountHolder || ""} onChange={(e) => set("accountHolder", e.target.value)} />
              </label>
              <label>
                Banco
                <input value={settings.bank || ""} onChange={(e) => set("bank", e.target.value)} />
              </label>
              <label>
                CLABE
                <input value={settings.clabe || ""} onChange={(e) => set("clabe", e.target.value)} />
              </label>
              <label>
                URL de QR
                <input value={settings.paymentQr || ""} onChange={(e) => set("paymentQr", e.target.value)} />
              </label>
            </fieldset>
          </div>
          ...
          <div className="settings-grid">
            <fieldset>
              ...
              <label className="inline-check">
                <input
                  type="checkbox"
                  checked={Boolean(settings.payments?.cardTerminal)}
                  onChange={(e) =>
                    set("payments", {
                      ...settings.payments,
                      cardTerminal: e.target.checked,
                    })
                  }
                />{" "}
                Habilitar tarjeta en caja
              </label>
              <label>
                Proveedor
                <select
                  value={settings.cardTerminal?.provider || "external"}
                  onChange={(e) =>
                    set("cardTerminal", {
                ...
                >
                  <option value="external">Terminal bancaria externa</option>
              ...
              <label>
                ...
                <input
                  value={settings.cardTerminal?.deviceName || ""}
                  onChange={(e) =>
                    set("cardTerminal", {
                      ...settings.cardTerminal,
...
function CustomersModule({ customers }) {
  const [query, setQuery] = useState("");
  const visible = customers.filter((c) => `${c.name} ${c.phone}`.toLowerCase().includes(query.toLowerCase()));
  return (
    <section className="dash-card data-card">
      <div className="module-toolbar">
        <label className="search">
...
function ReportsModule({ store }) {
  ...
  const orderHour = (order) => {
    if (Number.isInteger(order.createdHour)) return order.createdHour;
    const match = String(order.createdAt || "").match(/(\d{1,2})/);
    let hour = Number(match?.[1] || 0);
    if (/p\.?\s*m/i.test(order.createdAt || "") && hour < 12) hour += 12;
    if (/a\.?\s*m/i.test(order.createdAt || "") && hour === 12) hour = 0;
    return hour;
  };
  let rangeStart = referenceDate,
    rangeEnd = referenceDate;
  if (period === "semanal") {
    const date = new Date(`${referenceDate}T12:00:00`);
    const day = date.getDay() || 7;
    date.setDate(date.getDate() - day + 1);
    rangeStart = toISO(date);
  ...
  if (period === "mensual") {
    ...
    rangeEnd = toISO(new Date(date.getFullYear(), date.getMonth() + 1, 0));
  }
  if (period === "periodo") {
    rangeStart = from;
    rangeEnd = to;
    ...
    <div className="reports-module">
      <section className="dash-card report-filters">
        ...
        <div className="report-controls">
          ...
          {["turno", "diaria", "semanal"].includes(period) && (
            <label>
              Fecha de referencia
        ...
        <p className="range-label">
          <Clock3 /> Mostrando: <b>{rangeLabel}</b>
        </p>
      </section>
      <div className="report-summary">
        ...
        <article>
          ...
          <h3>{filtered.filter((o) => o.status === "Entregado").length}</h3>
        </article>
      </div>
      <div className="report-grid">
        <article className="dash-card">
          ...
          <p>
            <i className="cash-dot" /> Efectivo <b>{money(cash)}</b>
          </p>
          <p>
            <i className="card-dot" /> Transferencia / tarjeta <b>{money(card)}</b>
          </p>
        </article>
        <article className="dash-card">
          ...
          <div className="metric-line">
            <span>Pagados</span>
            <b>{filtered.filter((o) => o.paid).length}</b>
          </div>
          <div className="metric-line">
      ...
      <section className="dash-card report-detail">
        <div className="card-title">
          <div>
            <small>Detalle del reporte</small>
            <h2>Ventas incluidas</h2>
          </div>
          <b>{filtered.length} registros</b>
        </div>
        {filtered.length === 0 ? (
          <Empty text="No hay ventas en el periodo seleccionado" />
        ) : (
...
function PointOfSale({ store, close }) {
  ...
  const add = (p) =>
    setLines((rows) =>
      rows.some((r) => r.id === p.id)
        ? rows.map((r) => (r.id === p.id ? { ...r, qty: r.qty + 1 } : r))
        : [
            ...rows,
            {
              ...p,
              ...
              qty: 1,
              note: "",
              size: p.category === "Pizzas" ? "Mediana" : "",
            },
          ],
    );
      ...
      rows.map((line) =>
        line.id === id
          ? {
              ...line,
              size,
              price: pizzaPrice(line.basePrice || line.price, size),
            }
          : line,
      ),
    );
  const changeServiceType = (next) => {
    setServiceType(next);
    if (next === "Domicilio") setLines((rows) => rows.filter((line) => line.deliveryEnabled !== false));
  };
  ...
  useEffect(() => {
    if (timing === "Programar" && !scheduleSlots.includes(scheduledTime)) setScheduledTime(scheduleSlots[0] || "");
  ...
  const complete = async () => {
    ...
    try {
      const order = await store.createOrder({
        ...
        serviceType,
        lines: lines.map(({ id, name, price, qty, note, size, station }) => ({
          id,
          name,
          price,
          qty,
      ...
      if (settings.kitchenPrinterEnabled && kitchenLines.length)
        printAreaDocument({
          title: "COMANDA DE COCINA",
          printer: settings.kitchenPrinter,
          order,
          lines: kitchenLines,
          brand: store.business.brandName,
        });
      if (settings.barPrinterEnabled && barLines.length)
        setTimeout(
          () =>
            printAreaDocument({
              title: "COMANDA DE BARRA",
              printer: settings.barPrinter,
              order,
              lines: barLines,
              brand: store.business.brandName,
            }),
      ...
      await store.updateOrder(order.id, {
        drawerOpened,
        drawerOpenedAt: drawerOpened ? new Date().toISOString() : "",
        printLog: {
          cash: true,
          kitchen: settings.kitchenPrinterEnabled && kitchenLines.length > 0,
          bar: settings.barPrinterEnabled && barLines.length > 0,
        },
      });
  ...
  return (
    <Modal title={step === 5 ? "Venta confirmada" : "Nueva venta en caja"} close={close} large>
      {step < 5 && (
      ...
      {step === 1 && (
        <div className="pos-layout">
          <div>
            ...
            <h2>Captura los productos</h2>
            <div className="pos-products">
              {products
                .filter((product) => product.active && (serviceType !== "Domicilio" || product.deliveryEnabled !== false))
                .map((product) => (
                  <button key={product.id} onClick={() => add(product)}>
                    <ProductMedia product={product} />
                    <b>{product.name}</b>
                    <small>{money(product.price)}</small>
                    <em className={`area-tag ${product.station === "Ambas" ? "both" : ""}`}>{product.station === "Ambas" ? "Cocina + Barra" : product.station}</em>
                    {product.deliveryEnabled === false && <em>Solo recoger</em>}
                  </button>
                ))}
            </div>
          </div>
          <aside>
            ...
            {lines.length === 0 ? (
              <Empty text="Selecciona productos" />
            ) : (
              lines.map((line) => (
                <div className="pos-line-wrap" key={line.id}>
                  <div className="pos-line">
                    <div className="pos-qty">
                      <button type="button" onClick={() => changeLineQty(line.id, -1)}>
                        <Minus />
                      </button>
                      <span>{line.qty}</span>
                      <button type="button" onClick={() => changeLineQty(line.id, 1)}>
                  ...
                  <input value={line.note || ""} onChange={(e) => updateLineNote(line.id, e.target.value)} placeholder="Indicaciones: sin salsa, sin cebolla..." />
                </div>
              ))
            )}
            ...
            <button className="primary wide" disabled={!lines.length} onClick={() => setStep(2)}>
              Continuar a entrega y cliente <ArrowRight />
            </button>
          </aside>
        </div>
      )}
      {step === 2 && (
        <div className="pos-form-step">
          ...
          {serviceType && (
            <div className="form-grid pos-customer-form">
              <label className="full">
                Nombre del cliente
                <input
                ...
                <>
                  ...
                  <label>
                    ...
                    <input
                      type="number"
                      min="0"
                      value={customerData.deliveryFee}
                      onChange={(e) =>
                        setCustomerData({
                          ...customerData,
          ...
          <div className="schedule-card">
            ...
            <div className="service-options">
              ...
              <button className={timing === "Programar" ? "active" : ""} onClick={() => setTiming("Programar")}>
                <History /> Programar
              </button>
            </div>
            {timing === "Programar" && (
              <div className="schedule-fields">
                ...
                <label>
                  Selecciona horario
                  <select value={scheduledTime} onChange={(e) => setScheduledTime(e.target.value)} disabled={!scheduleSlots.length}>
                    {scheduleSlots.map((slot) => (
                      <option key={slot}>{slot}</option>
                    ))}
                  </select>
                </label>
                {!scheduleSlots.length && <p className="no-slots">No hay horarios disponibles para la fecha seleccionada.</p>}
              </div>
            )}
          </div>
          <div className="pos-form-actions">
            ...
            <button className="primary" disabled={!serviceType || !customerReady || !scheduleReady} onClick={() => setStep(3)}>
              Revisar cuenta <ArrowRight />
            </button>
          </div>
        </div>
      )}
      {step === 3 && (
        <div className="sale-review">
          ...
          <aside className="review-actions">
            ...
            <h2>Revisa la cuenta</h2>
            <ol>
              <li className="done">
                <Check /> {lines.reduce((sum, line) => sum + line.qty, 0)} productos capturados
              </li>
              <li className="done">
                <Check /> {serviceType}
              </li>
            ...
            <button className="review-back" onClick={() => setStep(2)}>
              Corregir datos
            </button>
          </aside>
        </div>
        ...
        <div className="payment-step">
          <section>
            ...
            <div className="payment-options">
              ...
              {settings.payments?.cardTerminal && (
                <button
                  className={payment === "Tarjeta en caja" ? "active" : ""}
                  onClick={() => {
                    setPayment("Tarjeta en caja");
                    setCashConfirmed(false);
                    setTransferValidated(false);
                  }}
                >
              ...
              {[
                ...
                .filter(([key]) => settings.payments?.[key])
                .map(([, label]) => (
                  <button
                    key={label}
                    className={payment === label ? "active" : ""}
                    onClick={() => {
                      setPayment(label);
                      setCashConfirmed(false);
                      setTransferValidated(false);
            ...
            {["Transferencia / Depósito", "Pago con QR", "CoDi", "DiMo"].includes(payment) && (
              <div className="transfer-panel">
                ...
                <div>
                  <small>Cuenta</small>
                  <b>{settings.accountNumber || "No configurada"}</b>
                </div>
                <div>
                  <small>CLABE</small>
                  <b>{settings.clabe || "No configurada"}</b>
                </div>
                {settings.cardNumber && (
                  <div>
                    <small>Tarjeta</small>
                ...
                <label className="full">
                  ...
                  {proof && <small>Registrado: {proof}</small>}
                </label>
          ...
          <aside className="payment-final">
            ...
            <button className="primary wide" disabled={!paymentReady || submitting} onClick={complete}>
              {submitting ? "Procesando..." : "Confirmar cobro y enviar pedido"} <ArrowRight />
            </button>
            <button className="review-back" onClick={() => setStep(3)}>
              Volver a revisar
            </button>
          </aside>
        </div>
        ...
        <div className="pos-success">
          ...
          <div className="print-results">
            <article>
              <Printer />
              <b>Ticket de caja</b>
              <small>Enviado a {settings.cashPrinter}</small>
            </article>
            {kitchenLines.length > 0 && (
              <article>
                <ChefHat />
                <b>Comanda de cocina</b>
                <small>{settings.kitchenPrinterEnabled ? `Enviada a ${settings.kitchenPrinter}` : "Impresora desactivada"}</small>
              </article>
            )}
            {barLines.length > 0 && (
              <article>
                <PackageCheck />
                <b>Comanda de barra</b>
                <small>{settings.barPrinterEnabled ? `Enviada a ${settings.barPrinter}` : "Impresora desactivada"}</small>
              </article>
            )}
            <article>
              <Wallet />
...
function BarView({ store }) {
  ...
  return (
    <DashboardShell active={active} setActive={setActive} moduleItems={[[active, PackageCheck]]} title="Barra / Despacho" subtitle="Productos listos o que no requieren pasar por cocina.">
      <BarBoard store={store} />
    </DashboardShell>
  );
}
...
function RealDeliveryMap({ orders, selectedOrder, onSelect }) {
  ...
  useEffect(() => {
    ...
    map.on("locationfound", (event) => {
      const next = {
        ...
        updatedAt: new Date().toLocaleTimeString("es-MX", {
          hour: "2-digit",
          minute: "2-digit",
          second: "2-digit",
        }),
      ...
      if (locationMarker.current) locationMarker.current.setLatLng(event.latlng);
        ...
        locationMarker.current = L.marker(event.latlng, {
          icon,
          zIndexOffset: 1000,
        })
          .addTo(map)
  ...
  useEffect(() => {
    ...
    orders.forEach((order, index) => {
      ...
      const icon = L.divIcon({
        className: `leaflet-order-icon ${selected ? "selected" : ""}`,
  ...
  return (
    <div className="real-map-shell">
      <div className="real-map-search">
        <Navigation />
        <div>
          <small>DESTINO SELECCIONADO</small>
          <b>{selectedOrder?.address || "Selecciona una entrega"}</b>
        </div>
      </div>
      <div ref={mapElement} className="leaflet-delivery-map" />
      <div className="real-map-status">
        <div>
          <span className={location ? "gps-live" : "gps-off"}>
            <Crosshair />
          </span>
          <div>
        ...
        <div className="real-map-actions">
          ...
          {selectedOrder && (
            <a href={`https://www.google.com/maps/dir/?api=1&destination=${encodeURIComponent(selectedOrder.address)}`} target="_blank" rel="noreferrer">
              <Navigation /> Navegar
            </a>
          )}
        </div>
  ...
  return (
    <main className="driver-view driver-portal">
      ...
      <nav className="driver-nav">
        {[
          ["Por entregar", PackageCheck],
          ["Ruta en mapa", Navigation],
          ["Historial", History],
        ].map(([label, Icon]) => (
          <button key={label} className={tab === label ? "active" : ""} onClick={() => setTab(label)}>
            <Icon /> {label}
            {label === "Por entregar" && <span>{activeOrders.length}</span>}
          </button>
        ))}
      </nav>
      {tab === "Por entregar" && (
        <section className="delivery-queue">
          <div className="queue-heading">
            ...
            <div className="queue-counters">
              ...
              <span>
                <i className="ready" /> Asignados {activeOrders.filter((o) => ["Listo para enviar", "Asignado a repartidor"].includes(o.status)).length}
              </span>
            </div>
          </div>
          {activeOrders.length === 0 ? (
            <Empty text="No tienes entregas pendientes" />
          ) : (
              ...
              {activeOrders.map((order, index) => (
                <article key={order.id}>
                  ...
                  <div className="delivery-actions">
                    ...
                    <a href={`sms:${order.phone}`}>
                      <MessageCircle /> Contactar
                    </a>
                    <a href={`https://www.google.com/maps/dir/?api=1&destination=${encodeURIComponent(order.address)}`} target="_blank" rel="noreferrer">
                      <Navigation /> Navegar
                    </a>
                  </div>
                  {["Listo para enviar", "Asignado a repartidor", "En ruta"].includes(order.status) ? (
                    <button className="primary wide" onClick={() => advance(order)}>
                      {order.status === "En ruta" ? "Confirmar entrega" : "Iniciar ruta"} <ArrowRight />
                    </button>
                  ) : (
      ...
      {tab === "Ruta en mapa" && (
        <section className="driver-route-layout">
          <div className="map-card route-map">
            <RealDeliveryMap orders={activeOrders} selectedOrder={selectedOrder} onSelect={setSelectedStop} />
          </div>
          <aside className="route-stops">
            <small>ORDEN DE ENTREGA</small>
            <h2>Paradas de la ruta</h2>
            {activeOrders.map((order, index) => (
              <button className={selectedOrder?.id === order.id ? "selected" : ""} key={order.id} onClick={() => setSelectedStop(order.id)}>
                <span>{index + 1}</span>
                <div>
                  <b>{order.customer}</b>
                  <small>{order.address}</small>
                </div>
                <strong className={`status ${order.status.toLowerCase().replaceAll(" ", "-")}`}>{order.status}</strong>
              </button>
            ))}
          </aside>
        </section>
      ...
      {tab === "Historial" && (
        <section className="dash-card driver-history">
          <div className="card-title">
            <div>
          ...
          {history.length === 0 ? (
            ...
            history.map((order) => (
              <article key={order.id}>
                ...
                <span className="status entregado">Entregado</span>
              </article>
            ))
          )}
        </section>
...
function Modal({ title, close, children, large }) {
  return (
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (alertSnapshot.current) {
      const hasNew = store.orders.some((order) => !alertSnapshot.current.has(order.dbId));
  ...
  const addItem = (product) => {
    setCart((rows) =>
      rows.some((r) => r.id === product.id)
        ? rows.map((r) => (r.id === product.id ? { ...r, qty: r.qty + 1 } : r))
        : [
            ...rows,
            {
              ...product,
              ...
              qty: 1,
              note: "",
              size: product.category === "Pizzas" ? "Mediana" : "",
            },
          ],
    );
    setShowCart(true);
      ...
      rows.map((row) =>
        row.id === id
          ? {
              ...row,
              size,
              price: pizzaPrice(row.basePrice || row.price, size),
            }
          : row,
      ),
    );
