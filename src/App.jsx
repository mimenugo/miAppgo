  ...
  return <>
    <main className="customer" id="inicio">
      ...
      <section className="menu-section" id="menu"><div className="section-heading"><div><span className="eyebrow">Nuestro menú</span><h2>Favoritos de la casa</h2></div><label className="search"><Search size={18}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Buscar platillo"/></label></div>
        ...
        <div className="product-grid">{visible.map(product=><article className="product-card" key={product.id}><div className={`product-image ${product.tone}`}><span>{product.emoji}</span><b><Star size={13} fill="currentColor"/> 4.9</b></div><div className="product-info"><small>{product.category}</small><h3>{product.name}</h3><p>{product.desc}</p><footer><strong>{money(product.price)}</strong><button onClick={()=>addItem(product)} aria-label={`Agregar ${product.name}`}><Plus size={19}/></button></footer></div></article>)}</div>
      </section>
    </main>
...
function ProductsModule({store}) {
  const empty={name:'',desc:'',price:'',category:'Tacos',emoji:'🍽️',active:true}
  ...
  return <><div className="module-toolbar"><p>{store.products.length} productos registrados</p><button className="primary" onClick={()=>setEditing(empty)}><Plus/> Nuevo producto</button></div><div className="product-admin-grid">{store.products.map(product=><article key={product.id}><span className={product.tone}>{product.emoji}</span><div><small>{product.category}</small><h3>{product.name}</h3><b>{money(product.price)}</b></div><label className="switch"><input type="checkbox" checked={product.active} onChange={()=>store.saveProduct({...product,active:!product.active})}/><i/></label><button className="icon-edit" onClick={()=>setEditing(product)}><Edit3/></button></article>)}</div>{editing&&<Modal title={editing.id?'Editar producto':'Nuevo producto'} close={()=>setEditing(null)}><div className="form-grid"><label>Nombre<input value={editing.name} onChange={e=>setEditing({...editing,name:e.target.value})}/></label><label>Precio<input type="number" value={editing.price} onChange={e=>setEditing({...editing,price:e.target.value})}/></label><label>Categoría<input value={editing.category} onChange={e=>setEditing({...editing,category:e.target.value})}/></label><label>Emoji<input value={editing.emoji} onChange={e=>setEditing({...editing,emoji:e.target.value})}/></label><label className="full">Descripción<input value={editing.desc} onChange={e=>setEditing({...editing,desc:e.target.value})}/></label></div><button className="primary wide modal-save" onClick={save}>Guardar producto</button></Modal>}</>
}
...
function PointOfSale({products,createOrder,close}) {
  ...
  return <Modal title="Nueva venta en caja" close={close} large><div className="pos-layout"><div><label className="field-label">Cliente<input value={customer} onChange={e=>setCustomer(e.target.value)}/></label><div className="pos-products">{products.filter(p=>p.active).map(p=><button key={p.id} onClick={()=>add(p)}><span>{p.emoji}</span><b>{p.name}</b><small>{money(p.price)}</small></button>)}</div></div><aside><h3>Detalle de venta</h3>{lines.length===0?<Empty text="Selecciona productos"/>:lines.map(line=><div className="pos-line" key={line.id}><span>{line.qty}×</span><b>{line.name}</b><strong>{money(line.price*line.qty)}</strong></div>)}<div className="pos-total"><span>Total</span><b>{money(subtotal)}</b></div><div className="payment-options compact"><button className={payment==='Efectivo'?'active':''} onClick={()=>setPayment('Efectivo')}><Wallet/> Efectivo</button><button className={payment==='Tarjeta'?'active':''} onClick={()=>setPayment('Tarjeta')}><CreditCard/> Tarjeta</button></div><button className="primary wide" disabled={!lines.length} onClick={complete}>Cobrar y notificar por WhatsApp</button></aside></div></Modal>
}
